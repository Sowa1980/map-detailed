...
# Ппрограмное обеспечение выбераем с открыым кодом:

1. Blender (должен стать редактором сквозная работа с (база данных-САКГШ).
2. JOSM
3. И так далее с открытым исходням кодом:
4. Полностью отказ от веб инструментов редактирования или портирование JOSM в веб (или аналог мульти платворменный разрабатываем свой ПО не должно ухудщать данные, как это есть в OSM. Будеи отказываться и поддерживать самое передовое. 

основываемся на самых востребованных не располяемся.

# Сайт:
Должен включать все матерьялы по проекту + тайлы карты быть единым цетром. Единый кол центр проекта и всех матерьялов.
