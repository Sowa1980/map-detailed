...
Пояснение такая схема существуют лишь до тех пор пока не будет Основной ЗД гео базы данных (после должны быть разработанны инструменты получения данных изнее по классу импорта в 2D, простая карта.
